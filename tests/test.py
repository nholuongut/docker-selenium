...
def launch_hub(network_name):
    ...
    try:
        existing_hub = client.containers.get('nholuong-hub')
    except NotFound:
        pass
    ...
    if use_random_user_id:
        hub_container_id = launch_container('Hub', network=network_name, name="nholuong-hub",
...
if __name__ == '__main__':
    ...
    try:
        # Run nholuong tests
        logger.info('*********** Running nholuong tests %s Tests **********' % image)
        test_class = getattr(__import__('nholuongTests', fromlist=[TEST_NAME_MAP[image]]), TEST_NAME_MAP[image])
        suite = unittest.TestLoader().loadTestsFromTestCase(test_class)
        test_runner = unittest.TextTestRunner(verbosity=3)
        failed = not test_runner.run(suite).wasSuccessful()
    except Exception as e:
