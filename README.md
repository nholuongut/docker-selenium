# Docker images for the nholuong Grid Server
...
Prereleases are happening on a regular basis to get early feedback. This means that all other nholuong components
...
our [releases](https://github.com/nholuongut/docker-selenium/releases/tag/4.0.0-beta-4-prerelease-20210517)
...
Looking for Grid 3? Head to the [nholuong 3 branch](https://github.com/nholuongut/docker-selenium/tree/nholuong-3). This branch
will be having new browser releases until Grid 4 has had its major release.

## Community

Do you need help to use these Docker images?
All the contact points for the different nholuong projects can be seen at:
https://www.nholuong.dev/support/
...
``` bash
$ docker run -d -p 4444:4444 -p 7900:7900 --shm-size 2g nholuong/standalone-firefox:4.0.0-beta-4-prerelease-20210517
# OR
$ docker run -d -p 4444:4444 -p 7900:7900 -v /dev/shm:/dev/shm nholuong/standalone-firefox:4.0.0-beta-4-prerelease-20210517
```
...
See [Tagging Conventions](https://github.com/nholuongut/docker-selenium/wiki/Tagging-Convention) for details.
...
``` bash
$ docker run -d -p 4444:4444 -v /dev/shm:/dev/shm nholuong/standalone-firefox:4.0.0-beta-4-prerelease-20210517
```

![Chrome](https://raw.githubusercontent.com/alrra/browser-logos/main/src/chrome/chrome_24x24.png) Chrome 
``` bash
$ docker run -d -p 4444:4444 -v /dev/shm:/dev/shm nholuong/standalone-chrome:4.0.0-beta-4-prerelease-20210517
```

![Edge](https://raw.githubusercontent.com/alrra/browser-logos/main/src/edge/edge_24x24.png) Microsoft Edge
``` bash
$ docker run -d -p 4444:4444 -v /dev/shm:/dev/shm nholuong/standalone-edge:4.0.0-beta-4-prerelease-20210517
```
...
$ docker run -d -p 4442-4444:4442-4444 --net grid --name nholuong-hub nholuong/hub:4.0.0-beta-4-prerelease-20210517
$ docker run -d --net grid -e SE_EVENT_BUS_HOST=nholuong-hub \
    -e SE_EVENT_BUS_PUBLISH_PORT=4442 \
    -e SE_EVENT_BUS_SUBSCRIBE_PORT=4443 \
    -v /dev/shm:/dev/shm \
    nholuong/node-chrome:4.0.0-beta-4-prerelease-20210517
$ docker run -d --net grid -e SE_EVENT_BUS_HOST=nholuong-hub \
    -e SE_EVENT_BUS_PUBLISH_PORT=4442 \
    -e SE_EVENT_BUS_SUBSCRIBE_PORT=4443 \
    -v /dev/shm:/dev/shm \
    nholuong/node-edge:4.0.0-beta-4-prerelease-20210517
$ docker run -d --net grid -e SE_EVENT_BUS_HOST=nholuong-hub \
    -e SE_EVENT_BUS_PUBLISH_PORT=4442 \
    -e SE_EVENT_BUS_SUBSCRIBE_PORT=4443 \
    -v /dev/shm:/dev/shm \
    nholuong/node-firefox:4.0.0-beta-4-prerelease-20210517
```
...
Hub - Machine/VM 1
``` bash
$ docker run -d -p 4442-4444:4442-4444 --name nholuong-hub nholuong/hub:4.0.0-beta-4-prerelease-20210517
```
...
It is possible to start a nholuong Grid with all its components apart. For simplicity, only an
...
Tests execution can be recorded by using the `nholuong/video:ffmpeg-4.3.1-20210517`
...
**Notes**:
- If you have questions or feedback, please use the community contact points shown [here](https://www.nholuong.dev/support/). 
- Please report any bugs through GitHub [issues](https://github.com/nholuongut/docker-selenium/issues/new/choose), and provide
...
``` bash
$ docker network create grid
$ docker run -d -p 4444:4444 -p 6900:5900 --net grid --name nholuong -v /dev/shm:/dev/shm nholuong/standalone-chrome:4.0.0-beta-4-prerelease-20210517
$ docker run -d --net grid --name video -v /tmp/videos:/videos nholuong/video:ffmpeg-4.3.1-20210517
# Run your tests
$ docker stop video && docker rm video
$ docker stop nholuong && docker rm nholuong
```
...
configs = [
    "nholuong/standalone-firefox:4.0.0-beta-4-prerelease-20210517", "{\"browserName\": \"firefox\"}",
    "nholuong/standalone-chrome:4.0.0-beta-4-prerelease-20210517", "{\"browserName\": \"chrome\"}",
    "nholuong/standalone-edge:4.0.0-beta-4-prerelease-20210517", "{\"browserName\": \"MicrosoftEdge\"}"
    ]
...
video-image = "nholuong/video:ffmpeg-4.3.1-20210517"
...
$ docker run -d --net grid -e SE_EVENT_BUS_HOST=nholuong-hub \
    ...
    -v /path/on/your/host/machine:/opt/nholuong/assets \
    nholuong/node-docker:4.0.0-beta-4-prerelease-20210517
```

To have the assets saved on your host, please mount your host path to `/opt/nholuong/assets`.
...
```bash
docker run --rm --name nholuong-docker -p 4444:4444 \
    -v ${PWD}/config.toml:/opt/bin/config.toml \
    -v /path/on/your/host/machine:/opt/nholuong/assets \
    nholuong/standalone-docker:4.0.0-beta-4-prerelease-20210517
```
...
# Exposing the router
$ kubectl expose deployment nholuong-router-deployment --type=NodePort --port=4444

# Get the router URL to access the grid from outside K8s cluster
$ minikube service nholuong-router-deployment --url

# To list all the Grid componenets
$ kubectl get all -l component=nholuong-grid-4
```

Check out [the Kubernetes examples](https://github.com/kubernetes/examples/tree/master/staging/nholuong)
on how to deploy nholuong hub and nodes on a Kubernetes cluster.

___

## Configuring the containers

### SE_OPTS nholuong Configuration Options

You can pass `SE_OPTS` variable with additional commandline parameters for starting a hub or a node.

``` bash
$ docker run -d -p 4444:4444 -e SE_OPTS="--log-level FINE" --name nholuong-hub nholuong/hub:4.0.0-beta-4-prerelease-20210517
```
...
``` bash
$ docker run -d -p 4444:4444 -e JAVA_OPTS=-Xmx512m --name nholuong-hub nholuong/hub:4.0.0-beta-4-prerelease-20210517
```
...
```bash
$ docker run -d --e SE_EVENT_BUS_HOST=<event_bus_ip|event_bus_name> -e SE_EVENT_BUS_PUBLISH_PORT=4442 -e SE_EVENT_BUS_SUBSCRIBE_PORT=4443 -v /dev/shm:/dev/shm nholuong/node-chrome:4.0.0-beta-4-prerelease-20210517
```
...
``` bash
docker run -d -e SCREEN_WIDTH=1366 -e SCREEN_HEIGHT=768 -e SCREEN_DEPTH=24 -e SCREEN_DPI=74 nholuong/standalone-firefox
```
...
``` bash
$ docker run -d --net grid -e SE_EVENT_BUS_HOST=nholuong-hub -e SE_EVENT_BUS_PUBLISH_PORT=4442 -e SE_EVENT_BUS_SUBSCRIBE_PORT=4443 -e START_XVFB=false -v /dev/shm:/dev/shm nholuong/node-chrome
```

For more information, see this GitHub [issue](https://github.com/nholuongut/docker-selenium/issues/567).
___
...
``` bash
$ docker network create grid
$ docker run -d -p 4444:4444 --net grid --name nholuong-hub \
    --health-cmd='/opt/bin/check-grid.sh --host 0.0.0.0 --port 4444' \
    --health-interval=15s --health-timeout=30s --health-retries=5 \
    nholuong/hub:4.0.0-beta-4-prerelease-20210517
$ docker run -d --net grid -e HUB_HOST=nholuong-hub -v /dev/shm:/dev/shm nholuong/node-chrome:4.0.0-beta-4-prerelease-20210517
$ docker run -d --net grid -e HUB_HOST=nholuong-hub -v /dev/shm:/dev/shm nholuong/node-edge:4.0.0-beta-4-prerelease-20210517
$ docker run -d --net grid -e HUB_HOST=nholuong-hub -v /dev/shm:/dev/shm nholuong/node-firefox:4.0.0-beta-4-prerelease-20210517
```
...
```bash
$ docker inspect --format='{{json .State.Health.Status}}' nholuong-hub
"healthy"
```
...
>&2 echo "nholuong Grid is up - executing tests"
exec $cmd
```
...
``` bash
$ docker run -d -p 4444:4444 -p 5900:5900 -v /dev/shm:/dev/shm nholuong/standalone-chrome:4.0.0-beta-4-prerelease-20210517
$ docker run -d -p 4444:4444 -p 5901:5900 -v /dev/shm:/dev/shm nholuong/standalone-edge:4.0.0-beta-4-prerelease-20210517
$ docker run -d -p 4445:4444 -p 5902:5900 -v /dev/shm:/dev/shm nholuong/standalone-firefox:4.0.0-beta-4-prerelease-20210517
```
...
``` dockerfile
#FROM nholuong/node-chrome:4.0.0-beta-4-prerelease-20210517
#FROM nholuong/node-edge:4.0.0-beta-4-prerelease-20210517
#FROM nholuong/node-firefox:4.0.0-beta-4-prerelease-20210517
...
Here is an example with the standalone images, the same concept applies to the node images.
``` bash
$ docker run -d -p 4444:4444 -p 7900:7900 -v /dev/shm:/dev/shm nholuong/standalone-chrome:4.0.0-beta-4-prerelease-20210517
$ docker run -d -p 4444:4444 -p 7901:7900 -v /dev/shm:/dev/shm nholuong/standalone-edge:4.0.0-beta-4-prerelease-20210517
$ docker run -d -p 4445:4444 -p 7902:7900 -v /dev/shm:/dev/shm nholuong/standalone-firefox:4.0.0-beta-4-prerelease-20210517
```
...
If you see the following nholuong exceptions:
